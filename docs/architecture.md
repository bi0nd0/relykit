...
The Nuxt login handler is also the OIDC third-party initiation boundary. It accepts GET and form POST, compares `iss` exactly with the configured issuer, rejects caller-selected target links, and then invokes the ordinary `startLogin` facade. The browser opening the initiation request therefore receives new application-owned state, nonce, and S256 PKCE rather than an emailed or provider-owned OAuth transaction. Local application login remains a GET with an independently validated local `returnTo`.
